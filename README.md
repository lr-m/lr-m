## 👾 About Me

Hi! I work in cyber and have a background in computer science. I primarily focus on exploit development, vulnerability research and reverse engineering.

[Check out my blog for in-depth details of my projects](https://luke-m.xyz)! 

## 📖 PagedOut Articles

<table>
  <tr>
    <td align="center">
      <a href="https://pagedout.institute/download/PagedOut_004_beta1.pdf">
        <img src="https://pagedout.institute/static/img/issue_4_cover_small.png" alt="PagedOut Issue 4" width="100">
      </a>
    </td>
    <td align="left">
      <strong>Issue 4:</strong>
      <ul>
        <li><a href="https://pagedout.institute/download/PagedOut_004_beta1.pdf#page=20"><em>You Wouldn't Gamble on a Router</em></a></li>
      </ul>
    </td>
  </tr>
  <tr>
    <td align="center">
      <a href="https://pagedout.institute/download/PagedOut_006.pdf">
        <img src="https://pagedout.institute/static/img/issue_6_cover_small.png" alt="PagedOut Issue 6" width="100">
      </a>
    </td>
    <td align="left">
      <strong>Issue 6:</strong>
      <ul>
        <li><a href="https://pagedout.institute/download/PagedOut_006.pdf#page=78"><em>strcpy(d,s); *cb-=4; // Gameboy</em></a></li>
      </ul>
    </td>
  </tr>
  <tr>
    <td align="center">
      <a href="https://pagedout.institute/download/PagedOut_007.pdf">
        <img src="https://pagedout.institute/static/img/issue_7_cover_small.png" alt="PagedOut Issue 7" width="100">
      </a>
    </td>
    <td align="left">
      <strong>Issue 7:</strong>
      <ul>
        <li><a href="https://pagedout.institute/download/PagedOut_007.pdf#page=65"><em>If It Has a Stream, It Can Play DOOM</em></a></li>
        <li><a href="https://pagedout.institute/download/PagedOut_007.pdf#page=58"><em>(Un)safe and Sound: Rooting a Camera with a Noise</em></a></li>
      </ul>
    </td>
  </tr>
</table>
